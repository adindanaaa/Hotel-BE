...
const booking = model.booking_order;
const detailBooking = model.booking_order_detail;
...
const addBookingRoom = async (req, res) => {
    try {
        const data = {
            ...
            id_guest: req.body.id_customer,
            id_tipe_kamar: req.body.id_tipe_kamar,
            bon_umber: req.body.bo_number,
            bo_date: req.body.bo_date,
            checkIn_date: req.body.checkIn_date,
            checkOut_date: req.body.checkOut_date,
            guest_name: req.body.guest_name,
            total_kamar: req.body.rooms_amount,
            bo_status: "baru",
        };

        // customer data
        const customerData = await guest.findOne({
        ...
        data.name_customer = customerData.guest_name
        data.email = customerData.email

        // rooms data
        let roomsData = await kamar.findAll({
            where: {
                id_room_type: data.id_tipe_kamar
            }
        });

        //room type data
        let roomTypeData = await tipeKamar.findAll({
        ...
        let dataBooking = await roomType.findAll({
            where: { id_room_type: data.id_tipe_kamar },
            include: [
                {
                    model: kamar,
                    as: "room",
        ...
        // get available rooms
        const bookedRoomIds = dataBooking[0].room.map((kamar) => kamar.id_kamar)
            ...
            for (let i = 0; i < dayTotal; i++) {
                for (let j = 0; j < roomsDataSelected.length; j++) {
                    ...
                    const dataDetailBooking = {
                        id_booking: result.id_booking_room,
                        id_room: roomsDataSelected[j].id_kamar,
